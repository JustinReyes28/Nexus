...
interface ButtonProps extends React.ButtonHTMLAttributes<HTMLButtonElement> {
  variant?: "primary" | "secondary" | "outline" | "ghost" | "ai" | "popular";
  size?: "sm" | "md" | "lg";
  isLoading?: boolean;
...
export const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant = "primary", size = "md", isLoading, leftIcon, rightIcon, asChild, children, ...props }, ref) => {
    ...
    const variants = {
      ...
      popular: "bg-indigo-600 hover:bg-indigo-700 border-indigo-600 text-white",
    };
    ...
    const organicStyles = variant === "outline" ? "border-b-4" : ""; // Remove hardcoded rounded-[8px] to allow size-specific radii
 
    if (asChild) {
      return React.cloneElement(children as React.ReactElement, {
        className: cn(baseStyles, variants[variant], sizes[size], organicStyles, className, (children as React.ReactElement).props.className)
      });
    }
